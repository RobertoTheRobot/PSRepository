...
$bin     = Get-ChildItem (Join-Path $PSScriptRoot src\bin)     -ErrorAction SilentlyContinue -Filter Confluent.Kafka.dll

# Binaries
foreach ($binary in $bin)
{
    Write-Verbose "Loading binary '$binary'"
    
    try{
        [System.Reflection.Assembly]::LoadFile($binary.FullName)
    }
    catch{
        Write-Error -Message "Failed to add-type dependant binary $($binary.FullName): $_"
    }
}
